...
import dev.michaili.anvilregionanalyzer.Networking.AnvilProfilingStream.ChunkMetadata;
...
public class ProfilingRender {
    ...
    public static void init() {
        ...
        HudRenderCallback.EVENT.register((drawContext, tickDelta) -> {
            ...
            if (lastReceivedData == null || lastReceivedData.getData().regionX() != pos.getRegionX() || lastReceivedData.getData().regionZ() != pos.getRegionZ()) {
                drawContext.drawText(textRenderer, "Waiting for data...", HUD_PADDING, HUD_PADDING + 10, 0xFFFFFF, true);
                return;
            }

            drawContext.drawText(textRenderer, "Region Size: " + humanReadableByteCountBin(lastReceivedData.getData().regionFileSize()) + " / " + (int) Math.ceil((double) lastReceivedData.getData().regionFileSize() / 4096L) + " sectors", HUD_PADDING, HUD_PADDING + 10, 0xFFFFFF, true);
            ...
            int mapHeight = (int) Math.ceil((double) (lastReceivedData.getData().regionFileSize() - 4096) / 4096 / HUD_REGION_MAP_WIDTH);
            ...
            var chunkData = lastReceivedData.getData().chunkMetadata();
            ...
            for (int i = 0; i < chunkData.length; i++) {
                ...
                for (int region = chunkMetadata.regionOffset() - 2; region < (chunkMetadata.regionOffset() - 2) + chunkMetadata.regionSize(); region++) {
                    ...
                    drawContext.fill(
                            ...
                            0x00ff8000 | ((chunkMetadata.lastSaveTime() == -1 ? 0 : Math.max(0, 255 - (int) Math.ceil((float) chunkMetadata.lastSaveTime() / SAVE_FLASH_DURATION * 255))) << 24)
                    );
                }
            }
        });
        ...
        WorldRenderEvents.LAST.register(context -> {
            context.gameRenderer().getClient().getProfiler().swap("anvilRegionAnalyzer");
            if (!shouldRender)
                return;
            ...
            for (int x = 0; x < 32; x++) {
                for (int z = 0; z < 32; z++) {
                    ...
                    if (lastReceivedData != null && lastReceivedData.getData().regionX() == playerPos.getRegionX() && lastReceivedData.getData().regionZ() == playerPos.getRegionZ())
    ...
    private static void renderChunkInfo(WorldRenderContext context, ChunkPos pos, @Nullable ChunkMetadata metadata) {
        ...
        //Vec3d targetPosition = new Vec3d(0, camera.getPos().y - 32, 0);
        {
            ...
            if (metadata != null && metadata.lastSaveTime() != -1) {
                ...
                opacity = Math.max(0.1f, 1f - ((float) metadata.lastSaveTime() / SAVE_FLASH_DURATION));
            }
        ...
        if (metadata != null) {
            ...
            String chunkDataSize = humanReadableByteCountBin(metadata.chunkDataSize());
            String sectorsActualSize = humanReadableByteCountBin(metadata.regionSize() * 4096L);
            String wastedSpace = humanReadableByteCountBin((metadata.regionSize() * 4096L) - metadata.chunkDataSize());

            var textRenderer = MinecraftClient.getInstance().textRenderer;
            var chunkPosTextX = - textRenderer.getWidth(chunkDataSize) / 2;
            var sizeTextX = - textRenderer.getWidth(metadata.regionSize() + " sectors (" + sectorsActualSize + ")") / 2;
            ...
            textRenderer.draw(metadata.regionSize() + " sectors (" + sectorsActualSize + ")", sizeTextX, -5, 0xFFFFFFFF, false, matrixStack.peek().getPositionMatrix(), context.consumers(), TextRenderer.TextLayerType.SEE_THROUGH, 0x0, 0);
